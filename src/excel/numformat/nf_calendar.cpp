	...
	if (nCalendarDisplayCode == CalendarDisplayCode::SHORT_QUARTER ||
		...
		case CalendarDisplayCode::SHORT_MONTH:
			value += 1; // month is zero based
			            // fall thru
		case CalendarDisplayCode::SHORT_DAY:
			swprintf(aStr, sizeof(aStr) / sizeof(wchar_t), L"%d",
		...
		case CalendarDisplayCode::LONG_MONTH:
			value += 1; // month is zero based
			swprintf(aStr, sizeof(aStr) / sizeof(wchar_t), L"%02d",
			         value); // #100211# - checked
			break;
		case CalendarDisplayCode::SHORT_YEAR:
...
bool DateTime::operator>(const DateTime &rDateTime) const {
	if ((Date::operator>(rDateTime)) || (Date::operator==(rDateTime) && Time::operator>(rDateTime)))
		return true;
	else
		return false;
}
...
void DateTime::GetWin32FileDateTime(sal_uInt32 &rLower, sal_uInt32 &rUpper) {
	...
	sal_Int64 nDays = nYears * 365 + nYears / 4 - nYears / 100 + nYears / 400 + GetDayOfYear() - 1;

	sal_Int64 aTime = a100nPerDay * nDays + a100nPerSecond * (sal_Int64(GetSec()) + 60 * sal_Int64(GetMin()) +
...
DateTime DateTime::CreateFromWin32FileDateTime(const sal_uInt32 &rLower, const sal_uInt32 &rUpper) {
	...
	sal_Int64 aTime = sal_Int64(sal_uInt64(rUpper) * SAL_CONST_UINT64(0x100000000) + sal_uInt64(rLower));
	...
	for (sal_Int64 nDaysCount = nDays; nDaysCount >= 0;) {
		nDays = nDaysCount;
		nMonths++;
		nDaysCount -= Date(1, nMonths, (sal_uInt16)(1601 + nYears)).GetDaysInMonth();
	}

	Date _aDate((sal_uInt16)(nDays + 1), nMonths, (sal_uInt16)(nYears + 1601));
...
static void DaysToDate(long nDays, sal_uInt16 &rDay, sal_uInt16 &rMonth, sal_uInt16 &rYear) {
	long nTempDays;
	long i = 0;
	bool bCalc;
...
sal_uInt16 Date::GetWeekOfYear(DayOfWeek eStartDay, sal_Int16 nMinimumNumberOfDaysInWeek) const {
	short nWeek;
	...
	} else // ( nMinimumNumberOfDaysInWeek == somehing_else, commentary examples
	       // for 4 )
	{
		// x_monday - thursday
...
long operator-(const Date &rDate1, const Date &rDate2) {
	sal_uIntPtr nTempDays1 = Date::DateToDays(rDate1.GetDay(), rDate1.GetMonth(), rDate1.GetYear());
	sal_uIntPtr nTempDays2 = Date::DateToDays(rDate2.GetDay(), rDate2.GetMonth(), rDate2.GetYear());
...
static sal_Int32 TimeToSec100(const Time &rTime) {
	...
	//	return (n100Sec + (nSec*100) + (nMin*60*100) + (nHour*60*60*100) *
	// nSign);
...
void Time::SetHour(sal_uInt16 nNewHour) {
	...
	nTime = (n100Sec + (nSec * 100) + (nMin * 10000) + (((sal_Int32)nNewHour) * 1000000)) * nSign;
}
...
void Time::SetMin(sal_uInt16 nNewMin) {
	...
	nTime = (n100Sec + (nSec * 100) + (((sal_Int32)nNewMin) * 10000) + (nHour * 1000000)) * nSign;
}
...
void Time::SetSec(sal_uInt16 nNewSec) {
	...
	nTime = (n100Sec + (((sal_Int32)nNewSec) * 100) + (nMin * 10000) + (nHour * 1000000)) * nSign;
}
...
void Time::Set100Sec(sal_uInt16 nNew100Sec) {
	...
	nTime = (((sal_Int32)nNew100Sec) + (nSec * 100) + (nMin * 10000) + (nHour * 1000000)) * nSign;
}
...
Time &Time::operator+=(const Time &rTime) {
	nTime = Sec100ToTime(TimeToSec100(*this) + TimeToSec100(rTime)).GetTime();
	return *this;
}
...
Time operator+(const Time &rTime1, const Time &rTime2) {
	return Sec100ToTime(TimeToSec100(rTime1) + TimeToSec100(rTime2));
}
...
#undef timezone
	...
	if ((nCacheSecOffset == -1) || ((nTicks - nCacheTicks) > 360000) || (nTicks < nCacheTicks) // handle overflow
	) {
		nTime = time(0);
